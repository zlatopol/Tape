...
TapeSorter::TapeSorter(TmpTapeGenerator tape_generator, Tape input, Tape output,
                       std::size_t max_memory)
    : tape_generator_(std::move(tape_generator)), input_(std::move(input)),
      output_(std::move(output)), max_int_cnt_(max_memory / sizeof(int)) {}
...
void TapeSorter::sort() {
  ...
  for (auto it = input_.begin(); it != input_.end();) {
    ...
    const auto file_size =
        std::min(input_.size(), it.offset() + max_int_cnt_ * sizeof(int)) -
        it.offset();
  ...
  while (tapes.size() != 1) {
    ...
    auto output_tape_path =
        tape_generator_.generate(fst_tape.size() + snd_tape.size());
...
Tape TapeSorter::getOutputTape() { return std::move(output_); }
