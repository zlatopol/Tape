...
Tape::Proxy::operator int() const
{
    return tape_->read();
}

Tape::Iterator::Iterator(Tape *tape, std::size_t offset) : tape_(tape), offset_(offset) {}

std::size_t Tape::Iterator::offset() const
{
    return offset_;
}

Tape::Proxy Tape::Iterator::operator*()
{
    return Proxy{tape_};
}
...
std::size_t Tape::max_offset() const
{
    return file_size_ - sizeof(int);
}

std::size_t Tape::offset() const
{
    return offset_;
}

Tape::Tape(std::filesystem::path file, std::size_t file_size, std::chrono::milliseconds delay) : file_(file, std::ios::binary | std::ios::in | std::ios::out), file_size_(file_size), delay_(delay)
{
    if (!file_.is_open())
    {
